...
#####################################################################################
#####################################################################################
def longitude_profile(wd, canv, fig, toolbar, lonf, gridNumEnt, varib, ptype, \
    ...
    print("\nPlot longitude profile of "+paramList[var_index]+" ...")
...
# set axis, labels, color bar etc for left three panels
    ...
    axes.set_ylabel (paramList[var_index], fontsize=2.8, labelpad=0.7)
    axes.text(0.01,1.01,add_text,ha='left',va='bottom',fontsize=2.6, \
        transform=axes.transAxes)

    #----- plot  ------
    if (var_index < 49 or var_index == 50 or var_index==51):
        axes.plot(lon, varib, lw=0.3, color='blue')
        axes.scatter(lon, varib, s=0.01, color='black')

    if (var_index == 49):
        colors=['black','red','blue','purple','yellow','green']
        labeltxt=['O+','H+','He+','O2+','N2+','NO+']
        for l in range(6):
            axes.plot(lon, varib[l][0:], lw=0.3, color=colors[l], label=labeltxt[l])
        ...
        if var_index == 66:
            ...
            labeltxt=[r'V$_{O^{+},\theta}$',r'V$_{H^{+},\theta}$',r'V$_{He^{+},\theta}$', \
                r'V$_{O^{+}_{2},\theta}$', r'V$_{N^{+}_{2},\theta}$',r'V$_{NO^{+},\theta}$']
        else:
            labeltxt=[r'V$_{O^{+},\phi}$',r'V$_{H^{+},\phi}$',r'V$_{He^{+},\phi}$', \
                r'V$_{O^{+}_{2},\phi}$', r'V$_{N^{+}_{2},\phi}$',r'V$_{NO^{+},\phi}$']
