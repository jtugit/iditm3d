...
inline double ele_cooling_rate(Field ***xx, double Te, double Tn, double ne, int i, int j, int k)
{
    ...
    * N2 rotation in eV s^-1 (not multiplied by ne yet) */
    ...
    Le[1]=2.99e-12*nN2*exp(fc*((Te-2000.0)/(2000.0*Te)))*(exp(gc*(Tn-Te)/(Te*Tn))-1.0);
    ...
    //original expression regards cooling as negative heating
    Le[4]=5.196e-13*nO2*exp(he*(Te-700.0)/(700.0*Te))*(exp(2770.0*(Tn-Te)/(Te*Tn))-1.0);
    ...
    //original expression regards cooling as negative heating
    Le[7]=1.57e-12*nO*exp(dd*(Te-3000.0)/(3000.0*Te))*(exp(22713.0*(Tn-Te)/(Te*Tn))-1.0);
